      ...
      if (variantTitle){
        ...
        this.recipeService.createVariant(variantTitle, this.recipe).pipe(
          ...
          switchMap(resultTuple => {
              ...
              this.ingredientQuantityMentionService.updateAllMentionPortions(ingredientQuantityList, recipe);
              return this.recipeService.update(recipe);
            }
          )
        ).subscribe( recipe => {
          const variantDetailsUrl = this.router.createUrlTree(['/recipe/details', recipe.slug]);
          this.snackBar.open('Variant Created !', 'Close', {duration: 2000, panelClass: ['green-snackbar']});
